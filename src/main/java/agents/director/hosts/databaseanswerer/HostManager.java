package agents.director.hosts.databaseanswerer;

import agents.director.mcp.clients.OracleSQLClient;
import agents.director.mcp.servers.OracleServer;
import agents.director.services.McpConfigLoader;
...
import agents.director.services.StreamingEventPublisher;
 ...
 */
public class HostManager extends AbstractVerticle {
    ...
    public void start(Promise<Void> startPromise) {
        ...
        configLoader.loadConfig()
            ...
            .onSuccess(v -> {
                ...
                System.out.println("[HostManager] Infrastructure deployed, waiting for components to be ready...");
                System.out.println("[HostManager] Expected servers: " + expectedServers);
                System.out.println("[HostManager] Expected clients: " + expectedClients);
                
                // Set up a startup timeout handler
                vertx.setTimer(30000, id -> {
                    if (!systemReady) {
                        System.err.println("[HostManager] WARNING: Startup timeout after 30 seconds!");
                        System.err.println("[HostManager] Missing components:");
                        ...
                        System.err.println("[HostManager] Ready servers: " + readyServers);
                        System.err.println("[HostManager] Ready clients: " + readyClients);
                        System.err.println("[HostManager] Total tools registered: " + allTools.size());
                        
                        // Log to CSV
                        vertx.eventBus().publish("log",
                            "Startup timeout - missing servers: " + missingServers + ", missing clients: " + missingClients + 
                            ",0,HostManager,Error,Startup");
                    }
                });
    ...
    private void registerEventBusConsumers() {
        ...
        System.out.println("[HostManager] Registering event bus consumers...");
        
        // Listen for tool discoveries from clients
        vertx.eventBus().consumer("mcp.tools.discovered", this::handleToolsDiscovered);
        System.out.println("[HostManager] Registered consumer for: mcp.tools.discovered");
        
        // Listen for client ready events
        vertx.eventBus().consumer("mcp.client.ready", this::handleClientReady);
        System.out.println("[HostManager] Registered consumer for: mcp.client.ready");
    ...
    private Future<Void> deployMcpInfrastructure() {
        ...
        return Future.all(serverDeployments)
            .compose(v -> {
                ...
                System.out.println("[HostManager] Waiting 5 seconds for servers to fully initialize HTTP endpoints...");
                vertx.setTimer(5000, id -> {
                    System.out.println("[HostManager] Server initialization wait complete, deploying clients...");
            ...
            .compose(v -> {
                ...
                if (oracleEnabled) {
                    System.out.println("[DEBUG] Deploying OracleSQLClient...");
                    expectedClients.add("oracle");
                    clientDeployments.add(deployVerticle(new OracleSQLClient(), null, "OracleSQLClient"));
                } else {
                ...
                if (localClientEnabled && !localServersConfig.isEmpty()) {
                    ...
                    if (!enabledLocalServers.isEmpty()) {
                        ...
                        // LocalServerClient deployment would go here if implemented
                    }
                }
    ...
    private Future<String> deployVerticle(AbstractVerticle verticle, DeploymentOptions options, String name) {
        ...
        System.out.println("[HostManager] Attempting to deploy: " + name);
        
        vertx.deployVerticle(verticle, options, ar -> {
            if (ar.succeeded()) {
                String deploymentId = ar.result();
                deploymentIds.add(deploymentId);
                System.out.println("[HostManager] Successfully deployed " + name + " [" + deploymentId + "]");
                
                // Log to CSV
                vertx.eventBus().publish("log",
                    "Deployed " + name + ",2,HostManager,Deployment,MCP");
                    
                promise.complete(deploymentId);
            } else {
                System.err.println("[HostManager] FAILED to deploy " + name + ": " + ar.cause().getMessage());
                ar.cause().printStackTrace();
                
                // Log failure to CSV
                vertx.eventBus().publish("log",
                    "Failed to deploy " + name + ": " + ar.cause().getMessage() + ",0,HostManager,Error,MCP");
    ...
    private void handleToolsDiscovered(Message<JsonObject> msg) {
        ...
        System.out.println("[HostManager] RECEIVED tools.discovered event:");
        System.out.println("[HostManager]   client: " + clientId);
        System.out.println("[HostManager]   server: " + serverName);
        System.out.println("[HostManager]   tools count: " + (tools != null ? tools.size() : "null"));
        
        // Log to CSV
        vertx.eventBus().publish("log",
            "HostManager received tools.discovered from " + clientId + " with " +
            (tools != null ? tools.size() : 0) + " tools,2,HostManager,Discovery,MCP");
        
        System.out.println("[DEBUG] HostManager.handleToolsDiscovered - Client " + clientId +
    ...
    private void handleClientReady(Message<JsonObject> msg) {
        ...
        // Add extensive logging
        System.out.println("[HostManager] RECEIVED client.ready event:");
        System.out.println("[HostManager]   clientId: " + clientId);
        System.out.println("[HostManager]   toolCount: " + toolCount);
        System.out.println("[HostManager]   type: " + status.getString("type", "unknown"));
        
        // Log to CSV
        vertx.eventBus().publish("log",
            "HostManager received client.ready from " + clientId + " with " +
            toolCount + " tools,2,HostManager,Registration,MCP");
        ...
        System.out.println("[HostManager] Ready clients: " + readyClients.size() + "/" + expectedClients.size());
    ...
    private void handleServerReady(Message<JsonObject> msg) {
        ...
        System.out.println("[HostManager] Server ready: " + serverName +
                         " on port " + status.getInteger("port"));
        System.out.println("[HostManager] Ready servers: " + readyServers.size() + "/" + expectedServers.size());
    ...
    private void routeToolCall(Message<JsonObject> msg) {
        ...
        vertx.eventBus().request("mcp.client." + clientId + ".call", clientRequest)
            .onSuccess(reply -> {
                ...
                result.put("_routedBy", "HostManager");
    ...
    private void checkIfSystemReady() {
        ...
        if (allServersReady && allClientsReady) {
            ...
            vertx.eventBus().publish("log", 
                "MCP infrastructure ready,1,HostManager,StartUp,MCP");
            ...
            System.out.println("[HostManager] Waiting for components:");
    ...
    public static void setRouter(Router parentRouter) {
        // List all clients
        parentRouter.get("/host/v1/clients").handler(HostManager::handleGetClients);
        
        // List all tools
        parentRouter.get("/host/v1/tools").handler(HostManager::handleGetTools);
        
        // Get system status
        parentRouter.get("/host/v1/mcp/status").handler(HostManager::handleGetStatus);
    }

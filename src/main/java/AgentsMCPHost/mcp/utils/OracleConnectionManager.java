        ...
        vertx.executeBlocking(blockingPromise -> {
            try (Connection conn = getConnection();
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    JsonArray results = resultSetToJson(rs);
                    blockingPromise.complete(results);
                }
                
            } catch (SQLException e) {
                blockingPromise.fail(new RuntimeException("Query execution failed: " + e.getMessage(), e));
            }
        }, res -> {
            if (res.succeeded()) {
    ...
    public Future<Integer> executeUpdate(String sql, Object... params) {
        ...
        vertx.executeBlocking(blockingPromise -> {
            try (Connection conn = getConnection();
                ...
                blockingPromise.complete(affectedRows);
                
            } catch (SQLException e) {
                blockingPromise.fail(new RuntimeException("Update execution failed: " + e.getMessage(), e));
            }
        }, res -> {
            if (res.succeeded()) {
